...
class CuboidPNPSolver(object):
    ...
    def solve_pnp(self, cuboid2d_points, pnp_algorithm = None):
        ...
        scale_axes_for_plotting = 7
        axes_points = np.asarray(
            [[1., 0., 0.,],
             [0., 1., 0.,],
             [0., 0., 1.,],
             [0., 0., 0.,],
            ])*scale_axes_for_plotting
        ...
        if is_points_valid:
            ...
            if ret:
                ...
                projected_axes, _ = cv2.projectPoints(axes_points, rvec, tvec, self._camera_intrinsic_matrix, self._dist_coeffs)
                projected_axes = np.squeeze(projected_axes)
        ...
        return location, quaternion, projected_points, projected_axes
